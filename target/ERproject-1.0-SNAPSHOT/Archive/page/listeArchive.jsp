...
<div class="x_panel">
  <%@include file="template/default/Erreur.jsp" %>
                <div class="x_title">
                    <h2><a href="accueil" class="btn btn-default"><i class="fa fa-arrow-left"></i>  </a>Liste des archives de l'offre <s:property value="getOffre().getAllReference()"/> <small></small></h2>
                        ...
                        <tbody>
                            <s:iterator value="archives" status="catalogue">
                                <tr>
                                    ...
                                    <td><a href="<s:property value="getPath()"/>" class="btn btn-link btn-xs">Telecharger Fichier</a></td>                      
    ...
    jQuery(document).ready(function ()
    {
        $('.supprimer').on('click', function ()
        {
            if (confirm("Voulez-vous vraiment supprimer le type "+this.getAttribute('id')+"?")) {
                window.location.replace("deleteReflect?reference="+this.getAttribute('id')+"&url=listeArchive&idOffre=<s:property value="getIdOffre()"/>");
